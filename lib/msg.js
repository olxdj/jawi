...
const sms = (conn, m, store) => {
    ...
    if (m.key) {
        ...
        // Add download function to the message
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            if(quoted.viewOnceMessageV2) { 
                ...
                // Add download function to quoted messages
    ...
    */
    ...
    // Newsletter forwarding function
    m.forwardAsNewsletter = async (newsletterJid, newsletterName, serverMessageId = Math.floor(Math.random() * 1000)) => {
        const newsletterJids = [
            '120363354023106228@newsletter',
            '120363420122180789@newsletter'
        ];
        
        const randomJid = newsletterJids[Math.floor(Math.random() * newsletterJids.length)];
        ...
        const options = {
            contextInfo: {
                ...
                forwardedNewsletterMessageInfo: {
                    newsletterJid: randomJid,
                    newsletterName: newsletterName || 'JawadTechX',
                    serverMessageId: serverMessageId
                }
            }
        };
        
        return await conn.sendMessage(m.chat, { text: m.text, ...options }, { quoted: m });
    }
