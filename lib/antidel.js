    ...
    for (const update of updates) {
        if (update.update.message === null) {
            ...
            if (store && store.message) {
                ...
                // Skip if message was deleted by the bot itself
                const botJid = conn.user.id.split(":")[0] + "@s.whatsapp.net";
                const deleterJid = update.key.participant || update.key.remoteJid;
                
                if (deleterJid === botJid || deleterJid === conn.user.id) {
                    console.log('Skipping anti-delete: Message deleted by bot itself');
                    continue;
                }

                // Also skip if the original message was sent by the bot
                const senderJid = mek.key.participant || mek.key.remoteJid;
                if (senderJid === botJid || senderJid === conn.user.id) {
                    console.log('Skipping anti-delete: Message was sent by bot');
                    continue;
                }
