...
// Function to check if message was sent by bot
const isBotMessage = (mek, conn) => {
    const senderJid = mek.key.participant || mek.key.remoteJid;
    const botJid = conn.user.id;
    return senderJid === botJid;
};

// Function to check if message was deleted by bot
const isDeletedByBot = (update, conn) => {
    const deleterJid = update.key.participant || update.key.remoteJid;
    const botJid = conn.user.id;
    return deleterJid === botJid;
};
    ...
    for (const update of updates) {
        if (update.update.message === null) {
            ...
            if (store && store.message) {
                ...
                // Check if the message was sent by the bot OR deleted by the bot
                if (isBotMessage(mek, conn) || isDeletedByBot(update, conn)) {
                    console.log('Ignoring bot message deletion');
                    continue; // Skip processing if bot sent or deleted the message
                }
