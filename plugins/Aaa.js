...
cmd({
    ...
    try {
        ...
        let caption = `*╭┈───〔 X VIDEOS 〕┈───⊷*\n`;
        
        results.forEach((item, index) => {
            const cleanTitle = item.title.replace(/[^\w\s]/gi, '').substring(0, 30);
            caption += `*┋ ⬡ ${index + 1}.* ${cleanTitle}\n`;
        });
        
        caption += `*╰─────────────────⊷*\n\n`;
        caption += `> ${toSmallCaps('please reply with 1 or 5')}`;
                ...
                if (selectedNum >= 1 && selectedNum <= 5) {
                    ...
                    try {
                        ...
                        await conn.sendMessage(sender, {
                            ...
                            fileName: `${videoData.title || 'X-VIDEO'}.mp4`,
                            mimetype: 'video/mp4',
                            caption: `*${videoData.title || 'X Video'}*`
                        }, { quoted: received });
