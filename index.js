...
BotActivityFilter(conn);
 ...
 /// READ STATUS       
  conn.ev.on('messages.upsert', async(mek) => {
    ...
    // Local LID to PN resolution function
    const resolveLidLocally = (jid, mek, isGroup = false) => {
        if (!jid || !jid.endsWith('@lid')) return jid;
        
        try {
            // Method 1: Check for alternate JID in the message key
            if (mek.key.remoteJidAlt && jid === mek.key.remoteJid) {
                return mek.key.remoteJidAlt;
            }
            if (mek.key.participantAlt && jid === mek.key.participant) {
                return mek.key.participantAlt;
            }
            
            // Method 2: Simple fallback - convert @lid to @s.whatsapp.net
            // This maintains the number part but changes the domain
            const numberPart = jid.split('@')[0];
            if (numberPart && /^\d+$/.test(numberPart)) {
                return numberPart + '@s.whatsapp.net';
            }
            
            return jid; // Return original if no resolution possible
        } catch (error) {
            console.log('LID resolution error:', error);
            return jid;
        }
    };

    // Function to resolve LIDs in mentionedJid array
    const resolveMentionedJids = (mentionedJid = [], mek) => {
        if (!Array.isArray(mentionedJid)) return mentionedJid;
        
        const resolved = [];
        for (const jid of mentionedJid) {
            if (jid && jid.endsWith('@lid')) {
                const resolvedJid = resolveLidLocally(jid, mek, true);
    ...
    // Updated sender detection with LID handling
    const getSender = (mek) => {
        if (mek.key.fromMe) {
            return conn.user.id.split(':')[0] + '@s.whatsapp.net' || conn.user.id;
        }
        
        const isGroup = mek.key.remoteJid.endsWith('@g.us');
        let sender = mek.key.participant || mek.key.remoteJid;
        
        // Resolve LID for sender
        sender = resolveLidLocally(sender, mek, isGroup);
  ...
  if (mek.key && newsletterJids.includes(mek.key.remoteJid)) {
    ...
    // Resolve mentioned JIDs for LIDs
    if (m.mentionedJid && Array.isArray(m.mentionedJid)) {
        m.mentionedJid = resolveMentionedJids(m.mentionedJid, mek);
    }
